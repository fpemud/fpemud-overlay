...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="alsa +arcade debug +mess opengl openmp tools X"
...
#	enable_feature USE_SYSTEM_LIB_LUA
	...
	if use X ; then
		disable_feature NO_X11
	else
		enable_feature NO_X11
	fi
	...
	f="3rdparty/bgfx/src/renderer_gl.h"
	sed -i "/glcontext_glx/i #define STR2(x) #x" $f
	sed -i "/glcontext_glx/i #define STR(x) STR2(x)" $f
	sed -i "/glcontext_glx/i #pragma message("1 = " STR(BGFX_CONFIG_RENDERER_OPENGLES))" $f
	sed -i "/glcontext_glx/i #pragma message("2 = " STR(BGFX_CONFIG_RENDERER_OPENGL))" $f
}
